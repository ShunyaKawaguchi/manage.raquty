...
if ($_SESSION['nonce_id'] == $_POST['raquty_nonce']) {
    if (isset($_POST['topics_id'])) {
        ...
        $log_before = 'topics_id:'.$topics_id.'/date:'.$get_log_before['post_date'].'/status:'.$get_log_before['post_status'].'/title:'.$get_log_before['post_title'].'/contents:'.$get_log_before['post_contents'];
        $log_after = 'topics_id:'.$topics_id.'/date:'.$post_date.'/status:'.$post_status.'/title:'.$post_title.'/contents:'.$post_content;

        // パラメーターをバインド
        $sql = 'UPDATE tournament_topics SET post_title = ?, post_content = ?, post_status = ?,post_date = ?,post_modified = NOW() WHERE post_id = ?';
        ...
        if ($stmt->execute()) {
            ...
            add_log('update_tournament_topics_status' , h($_POST['tournament_id']), null ,  h($_POST['log_before']) , $post_status);
    
            //リダイレクト
            echo "<script>
                    window.onload = function() {
                        const message = '公開状況の更新が完了しました。';
                        alert(message);
