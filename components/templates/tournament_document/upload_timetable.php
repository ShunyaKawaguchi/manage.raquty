...
if ($_SESSION['nonce_id'] == $_POST['raquty_nonce']) {
    ...
    $upload_dir = '../../../files/document/timetable/';
    $upload_dir_db = 'files/document/timetable/';
    ...
    // ファイルをサーバーに保存する
    if(move_uploaded_file($_FILES['timetable']['tmp_name'], $file_path)) {
        if(get_document_path_tf($tournament_id, 'timetable')){
            $before_path = get_document_path_return(h($_POST['tournament_id']), 'timetable');
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    //データ更新完了
                    //ログ挿入
                    add_log('upload_timetable' , $tournament_id, null , $before_path['document_path'], $file_path_db);
                    //リダイレクト
                    echo '<script>alert("日程表のアップロードが完了しました。");</script>';
                    ?>
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    //ログ挿入
                    add_log('upload_timetable' , $tournament_id, null , null, $file_path_db);
                    //リダイレクト
                    echo '<script>alert("日程表のアップロードが完了しました。");</script>';?>
                    <script> window.location.href = "<?= home_url('Tournament/View/Document?tournament_id=').$tournament_id ?>";</script>'; 
                    <?php
