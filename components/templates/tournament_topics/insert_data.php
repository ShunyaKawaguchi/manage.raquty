...
// INSERTクエリを実行
$insertQuery = "INSERT INTO tournament_topics (tournament_id, post_title, post_date, post_modified, post_status,post_author) VALUES (?, '名称未設定のトピックス', NOW(), NOW(), 'draft',?)";
$stmt = $cms_access->prepare($insertQuery);

if ($stmt) {
    $stmt->bind_param("ii",h($_POST['tournament_id']),$_SESSION['group_id']);
